...
func NewAuthService(
	...
	if config.JWTExpiration == 0 {
		config.JWTExpiration = JWT_EXPIRE_TIME
	}
...
func (s *AuthServiceImpl) Login(ctx context.Context, req *LoginRequest) (*LoginResponse, error) {
	...
	// Generate JWT token
	token, err := s.tokenService.GenerateJWT(user.ID, s.config.JWTExpiration)
	if err != nil {
		log.WithFields(log.Fields{
			"user_id": user.ID,
			"error":   err.Error(),
...
func (s *AuthServiceImpl) Register(ctx context.Context, req *RegisterRequest) (*RegisterResponse, error) {
	...
	if maxLen == 0 {
		maxLen = ALLOW_MAX_LENGTH_OF_USERNAME
	}
	if len(req.Username) > maxLen {
		return nil, fmt.Errorf("%w: username must be less than %d characters",
			ErrInvalidInput, maxLen)
	}
	...
	// Generate JWT token
	token, err := s.tokenService.GenerateJWT(userID, s.config.JWTExpiration)
	if err != nil {
		log.WithFields(log.Fields{
			"user_id": userID,
			"error":   err.Error(),
...
func (s *AuthServiceImpl) GenerateToken(userID int) (string, error) {
	return s.tokenService.GenerateJWT(userID, s.config.JWTExpiration)
}
